<template>
  <div class="add-listing-block">
    <div class="alert alert-info z-depth-1" :class="{ active : success }" v-if="success == true" role="alert">
                ...
                <div class="package-block">
                  <div v-for="(package, index) in packages">
                      ...
                      <div class="row">
                        <div class="col-md-6"> 
                         ...
                         <div class="col-md-6"> 
                          ...
                          <input type="text" v-model="package.price" class="form-control"/>
                    ...
                    <div class="form-group"> 
                      ...
                      <textarea v-model="package.description" class="form-control"></textarea>
                    </div> 
                  </div>  
                    ...
                    <div v-for="(item, index) in gallery">
                      <div class="col-sm-4">
                        <div class="add-listing-image">
                          <div v-if="item.image.slice(0, 4) == 'data'">
                            <img :src="item.image" class="img-responsive" alt="add-listing-img">
                          </div>
                          <div v-if="item.image.slice(0, 4) != 'data'">
                            <img :src="$props.gallery_path+'/'+item.image" class="img-responsive" alt="add-listing-img">
                          </div>
                ...
                <div class="form-btn-section">
                  <div class="upload-img-btn">
                    <div class="form-group input-file-block">
                      <input v-on:change="onFileChange" type="file" id="photo" class="input-file">
                      <label for="photo" class="js-labelFile btn btn-pink" data-toggle="tooltip" data-original-title="Profile Pic">
...
<script>
  export default {
    props: ['listing_id', 'categories_url', 'country_url', 'state_url', 'city_url', 'icon_path', 'manage_listing_api', 'gallery_path', 'update_path'],
    data () {
      return {
    ...
    created () {
      ...
      this.fetchStates();
      this.fetchCities();
      this.listing_all_data();
    },

    methods: {

        listing_all_data() {
          this.$http.get(this.$props.manage_listing_api).then((response) => {
            console.log(response);
            this.listing = response.data.listing;
            this.listing.category_id = response.data.category;
            this.listing.listing_country_id = response.data.country;
            this.listing.listing_state_id = response.data.state;
            this.listing.listing_city_id = response.data.city;
            this.mapCenter.lat = parseFloat(response.data.listing.latitude);
            this.mapCenter.lng = parseFloat(response.data.listing.longitude);
            this.markers = [{
              position: {
                lat: parseFloat(response.data.listing.latitude),
                lng: parseFloat(response.data.listing.longitude)
              }
            }];
        ...
        fetchCategories() {
          this.$http.get(this.$props.categories_url).then(response => {
        ...
        fetchCountries() {
          this.$http.get(this.$props.country_url).then(response => {
        ...
        fetchStates() {
          this.$http.get(this.$props.state_url).then(response => {
        ...
        fetchCities() {
          this.$http.get(this.$props.city_url).then(response => {
        ...
        add_listing() {
          ...
          this.$http.patch(this.$props.update_path, {listing: result, packages: this.packages, galleries: this.gallery}).then((response) => {
            console.log(response);
