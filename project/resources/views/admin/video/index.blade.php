...
@section('content')
  <div class="content-main-block  mrg-t-40">
    <div class="admin-create-btn-block">
      <a href="{{route('video.create')}}" class="btn btn-danger btn-md">Add Video</a>
    <!-- Delete Modal -->
        ...
        <div class="modal-dialog modal-sm">
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              {!! Form::open(['method' => 'POST', 'action' => 'VideoController@bulk_delete', 'id' => 'bulk_delete_form']) !!}
          ...
          <tbody>
            @foreach($video as $key => $item)
              <tr>
                ...
                <td>
                  @if ($item->image != null)
                    <img src="{{ asset('images/video/'.$item->image) }}" class="img-responsive" width="80" alt="image">
                  @else
                    N/A  
                  @endif
                </td>
                ...
                <td>{{$item->title}}</td>
                <td>{{str_limit($item->link != NULL ? $item->link : '-',40)}}</td>
                <td>
                  <div class="admin-table-action-block">
                    <a href="{{route('video.edit', $item->id)}}" data-toggle="tooltip" data-original-title="Edit" class="btn-info btn-floating"><i class="material-icons">mode_edit</i></a>
                    <!-- Delete Modal -->
                    ...
                    <div id="{{$item->id}}deleteModal" class="delete-modal modal fade" role="dialog">
                      <div class="modal-dialog modal-sm">
                        ...
                        <div class="modal-content">
                          ...
                          <div class="modal-footer">
                            {!! Form::open(['method' => 'DELETE', 'action' => ['VideoController@destroy', $item->id]]) !!}
                                <button type="reset" class="btn btn-gray translate-y-3" data-dismiss="modal">No</button>
                                <button type="submit" class="btn btn-danger">Yes</button>
                            {!! Form::close() !!}
                          </div>
